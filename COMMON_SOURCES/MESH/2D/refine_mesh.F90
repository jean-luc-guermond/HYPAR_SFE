...
!=== Loop on outer edges, not owned by this proc ===!
   ...
   SUBROUTINE refinement_iso_grid_distributed(mesh_p1)
      ...
      mesh%me = refine_order**2 * mesh_p1%me
      mesh%mes = refine_order * mesh_p1%mes !---> something with news to take into account
      mesh%mes_int = refine_order * mesh_p1%mes_int
      mesh%np = mesh_p1%np + (mesh_p1%medge + mesh_p1%medges)*(refine_order - 1) + mesh_p1%me*(refine_order-1)*(refine_order-2)/2
      mesh%medge = refine_order * mesh_p1%medge + 3*(refine_order-1)*refine_order/2*mesh_p1%me
      mesh%medges = refine_order * mesh_p1%medges
      ...
      DO m = 1, me !===loop on the elements unrefined mesh
         ...
         DO i = 1, nw
            ...
            mesh%jce(i, m_center) = mesh%disedge(proc) - 1 + refine_order * mesh_p1%medge &
      ...
      !      END DO
      !===In the mean time
      mesh%mextra = mesh_p1%mextra * refine_order**2
      ...
      DO m=1, mesh_p1%mes_extra
         ...
         END DO
         ! mextra = mextra + 1
         ! mesh%jjs_extra(:, mextra) = mesh_p2%jjs_extra(jjs_HL(1, :), (mextra+1)/2)
         ! mextra = mextra + 1
         ! mesh%jjs_extra(:, mextra) = mesh_p2%jjs_extra(jjs_HL(2, :), (mextra+1)/2)
      END DO
