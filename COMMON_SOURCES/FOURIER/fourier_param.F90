MODULE fourier_param_module
  ...
  TYPE fourier_param_type
     ...
     INTEGER :: Nmax_real, Nmax_pad, Nmax_pad_real
     REAL(KIND=8) :: dx, dx_pad
     REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: rr
     REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: rr_pad
   CONTAINS
...
CONTAINS
  SUBROUTINE init_fourier_param(this)
    ...
    this%Nmax_pad = (3*this%Nmax)/2
    this%Nmax_pad_real = 2*this%Nmax_pad-1
    this%dx_pad = this%length/this%Nmax_pad_real
    ALLOCATE(this%rr_pad(this%Nmax_pad_real))
    DO i = 1, this%Nmax_pad_real
       this%rr_pad(i) = (i-1)*this%dx_pad
    END DO
