...
MODULE template_cell_limiting_engine_parallel_module
   ...
   TYPE ::  limiting_functional_type
      ...
      PROCEDURE(template_specialized_iterative_cell_limiting_procedure), POINTER, NOPASS :: spe_iterative_cell_limiting_procedure => NULL()
...
#:def spe_limiteur(syst_dim)
      SUBROUTINE template_specialized_iterative_cell_limiting_procedure(limiter, xx_in, loc_min)
         USE space_dim
         IMPORT :: limiting_type
         IMPLICIT NONE
         CLASS(limiting_type),                                 INTENT(IN)    :: limiter
         REAL(KIND=8), DIMENSION(:),                           INTENT(IN)    :: loc_min
         REAL(KIND=8), DIMENSION(SIZE(loc_min),${syst_dim}$),  INTENT(INOUT) :: xx_in
      END SUBROUTINE template_specialized_iterative_cell_limiting_procedure
#:enddef
$:spe_limiteur(syst_dim)
   END INTERFACE
...
#include "petsc/finclude/petsc.h"
      ...
      LOGICAL,      DIMENSION(SIZE(xx_in,1))               :: mask_up_vec, mask_down_vec
      ...
      mask_up_vec   = psi(xx_in,loc_min + this%epsilon*ABS(loc_min(:)))>0
      mask_down_vec = psi(xx_in,loc_min - this%epsilon*ABS(loc_min))<0
      ...
      DO m = 1, me
         ...
         mask_up   = mask_up_vec(jloc)
         mask_down = mask_down_vec(jloc)
   ...
   SUBROUTINE RELAX_BOUNDS(this, state, psi_min, nth_fonctional)
      ...
      CASE(RELAX_MINMOD) !===Minmod
         ...
         END DO
         denom = denom*mask
         CALL this%mesh%reduce_through_ghost(denom, MPI_MIN)
      CASE DEFAULT
...
END MODULE template_cell_limiting_engine_parallel_module
