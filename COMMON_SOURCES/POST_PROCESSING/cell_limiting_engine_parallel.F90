...
CONTAINS
  ...
  SUBROUTINE iterative_cell_limiting_procedure(this,xx_in,loc_min,psi,zero_of_psi,xx_out)  
    ...
    INTEGER :: i, k, m, n, me, nw, syst_size, iminus, iplus, comp
    ...
    DO m = 1, me
       ...
       IF (SUM(limit_zero+limit_plus+limit_minus).NE.nw) THEN
          ...
          STOP
       END IF
       !write(*,*)  'm', m, 'iplus*iminus', iplus*iminus, iminus, iplus
    ...
    !TESTTTTTTTTTTTT
    if (test) then
       DO n = 1, size(xx_out,1)
          write(*,*) 'n', n, xx_in(n,1), xx_out(n,1)
       END DO
       write(*,*) ' after limiting', SUM(abs(xx_in(:,1)-xx_inter(:,1)))
       stop
    END if
    !TESTTTTTTTTTTTT
