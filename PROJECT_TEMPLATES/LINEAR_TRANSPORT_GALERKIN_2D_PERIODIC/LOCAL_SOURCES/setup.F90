MODULE setup
   ...
   PRIVATE
   REAL(KIND=8), PARAMETER :: x0=0.0d0, y0=0.d0, a = 0.3d0, cx=1.d0, cy=2.d0
CONTAINS
...
!==========================================================================
   ...
   FUNCTION rho_circle(this, time, rr) RESULT(vv)
      ...
      INTEGER :: n, k_x, k_y
      REAL(KIND=8) :: xt, yt, length = 2.d0 !<== FIXME

      DO n=1, SIZE(rr,2)

         k_x = floor((rr(1, n) + 1.d0 - cx*time)/length)
         k_y = floor((rr(2, n) + 1.d0 - cy*time)/length)

         xt = rr(1, n) - cx*time -k_x*length
         yt = rr(2, n) - cy*time -k_y*length
