...
!>> limited global uses to avoid unexpected behaviors
   ...
   TYPE argument_linear_transport_type
      ! CHARACTER(LEN=rec_length) :: cx = "=== Linear transport cx ==="
      ! CHARACTER(LEN=rec_length) :: cy = "=== Linear transport cy ==="
...
CONTAINS
   ...
   !   !===c_transport
   !   CALL read_data(argument_data%c_transport, this%c_transport, opt_name=this%name)
