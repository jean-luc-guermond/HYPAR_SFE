...
#include "petsc/finclude/petsc.h"
  ...
  TYPE argument_setup_data_type
      ...
      CHARACTER(LEN=rec_length) :: char_which_init    = '=== Which initial condition? (bump, sine, step) ==='
  ...
  TYPE setup_data_type
     ...
     CHARACTER(LEN=rec_length) :: char_which_init = 'bump'
     INTEGER        :: which_init
     INTEGER        :: syst_size
   CONTAINS
  ...
  CHARACTER(LEN=20), DIMENSION(3), PRIVATE, PARAMETER  :: list_init_rho = &
              [CHARACTER(LEN=20) :: 'bump', 'sine', 'step']
...
CONTAINS
  ...
  SUBROUTINE start_setup
    ...
    CALL init_state_functions(linear_transport, setup_data%which_init)
