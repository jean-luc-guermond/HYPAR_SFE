...
def generate_user_case(path_hypar, name_developper_case, fe_dim, exist_ok=False):
    ...
    for spe_case in list_spe_case:
        ...
        try:
            ...
            for file_F90 in (path_dvp_case / spe_case / "LOCAL_SOURCES").glob('*.F90'):
                shutil.copy(file_F90, path_user_case / spe_case / "LOCAL_SOURCES" / file_F90.name)
            for file_inc in (path_dvp_case / spe_case / "LOCAL_SOURCES").glob('*.inc'):
                shutil.copy(file_inc, path_user_case / spe_case / "LOCAL_SOURCES" / file_inc.name)
            ...
            shutil.copy(path_in, path_out_var_cmake)
            ...
            if (os.path.exists(path_py_precompile_in)):
                shutil.copy(path_py_precompile_in, path_py_precompile_out)
